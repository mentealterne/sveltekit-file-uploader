...
export const resourceSchema = z.object({
	title:z.string().min(1,{error:"Title must be at least 1 character"}).max(200,{error:"Title must be at most 200 characters"}),
	...
	resourceFile:z.instanceof(File)
		.refine((file)=>file.size>0,{message:"A file is required"})
		.refine((file)=>file.size<=fileSizeLimit,{message:`File size must be less than ${fileSizeLimit / (1024 * 1024)} MB`})
		.refine((file)=>allowedFileTypes.includes(file.type),{message:"File type is not supported"})
})
